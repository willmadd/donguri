    ...
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="mt-2 text-sm text-sumi-soft">
          A word is <span className="font-medium text-sumi">learnt</span> once
          you've been introduced to it, and becomes{" "}
          <span className="font-medium text-sumi">known</span> once you answer
          it correctly four times in a row during practice — getting it wrong
          sends it back to the start, so keep at it to lock words in.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4">
        {lessons
          .filter((l) =>
            ["colours", "everyday basics", "foods"].includes(
              l.title.toLowerCase(),
            ),
          )
          .map((lesson) => {
            const complete =
              lesson.totalWords > 0 && lesson.knownWords === lesson.totalWords;
            const learntPercent =
              lesson.totalWords > 0
