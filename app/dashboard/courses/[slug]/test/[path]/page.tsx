...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return { title: `Test ${kind === "grammar" ? "grammar" : "vocabulary"} — ${course.title}` };
}
...
export default async function TestPage({ params }: PageProps) {
  ...
  const [{ course }, quiz, profile] = await Promise.all([
    getCourseHome(slug),
    getTestQueueForCourse(slug, kind),
    requireProfile(),
  ]);
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-sumi/10 bg-washi-soft px-6 py-24 text-center">
          ...
          <p className="max-w-sm text-sm text-sumi-soft">
            Learn a few {label === "grammar" ? "points" : "words"} first — they&apos;ll show up
            here for review.
          </p>
          <Link
            href={`/dashboard/courses/${slug}/learn/${kind}`}
